...
:- pred infinity(num(N)) <= arithmetic.arithmetic(N).
:- mode infinity(in(infinity)) is det.
:- mode infinity(in(basic_num)) is failure.
:- mode infinity(in) is semidet.

:- pred negative(num(N)::in) is semidet <= arithmetic.arithmetic(N).
:- pred positive(num(N)::in) is semidet <= arithmetic.arithmetic(N).
...
:- mode abs(in(infinity))  = out(infinity) is det.
...
:- pred (num(N)::in) <  (num(N)::in) is semidet <= arithmetic.arithmetic(N).
:- pred (num(N)::in) =< (num(N)::in) is semidet <= arithmetic.arithmetic(N).
:- pred (num(N)::in) >  (num(N)::in) is semidet <= arithmetic.arithmetic(N).
:- pred (num(N)::in) >= (num(N)::in) is semidet <= arithmetic.arithmetic(N).
...
abs(N) = ( if negative(N) then -N else N ).
