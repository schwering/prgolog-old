...
:- type lp_terms == list(lp_term).
    ...
    %
:- func lpq.lp_term(lp_var) = lp_term.
...
:- func lpq.make_vars_eq_constraint(lp_var, lp_var) = constraint.
    ...
    %
:- func lpq.make_var_const_eq_constraint(lp_var, rat) = constraint.
:- func lpq.make_var_const_gte_constraint(lp_var, rat) = constraint.

    % Create a constraint that is trivially false.
    %
:- func lpq.false_constraint = constraint.

    % Create a constraint that is trivially true.
    %
:- func lpq.true_constraint = constraint.

    % Succeeds if the constraint is trivially false.
    %
:- pred lpq.is_false(constraint::in) is semidet.

    % Succeeds if the constraint is trivially true.
    %
:- pred lpq.is_true(constraint::in) is semidet.
    ...
    %
:- pred lpq.restore_equalities(constraints::in, constraints::out)
    is det.
...
:- pred remove_some_entailed_constraints(lp_varset::in, constraints::in,
    ...
    %
:- pred lpq.inconsistent(lp_varset::in, constraints::in) is semidet.
    ...
    %
:- func lpq.simplify_constraints(constraints) = constraints.
    ...
    %
:- func lpq.substitute_vars(lp_vars, lp_vars, constraints)
    = constraints.
:- func lpq.substitute_vars(map(lp_var, lp_var), constraints)
    = constraints.

    % Make the values of all the variables in the set zero.
    %
:- func lpq.set_vars_to_zero(set(lp_var), constraints) = constraints.
...
:- type lp_result
    ...
    %
:- func lpq.solve(constraints, direction, objective, lp_varset)
    = lp_result.
...
:- type projection_result
    ...
    %
:- func lpq.project(lp_vars, lp_varset, constraints)
    = projection_result.
:- pred lpq.project(lp_vars::in, lp_varset::in, constraints::in,
    ...
    %
:- pred lpq.project(lp_vars::in, lp_varset::in, maybe(int)::in,
...
:- type entailment_result
    ...
    %
:- func lpq.entailed(lp_varset, constraints, constraint) =
    entailment_result.
    ...
    %
:- pred lpq.entailed(lp_varset::in, constraints::in,
...
:- type output_var == (func(lp_var) = string).
    ...
    %
:- pred lpq.output_constraints(output_var::in, constraints::in,
    io::di, io::uo) is det.
...
%
    ...
    %
:- pred lpq.write_constraints(constraints::in, lp_varset::in, io::di,
    io::uo) is det.
...
is_true(eq([],  Const)) :- Const =  rat.zero.
    ...
    %
:- func lpq.standardize_constraints(constraints) = constraints.
...
standardize_constraints(Constraints) =
    ...
    %
:- func lpq.standardize_constraint(constraint) = constraint.
...
inconsistent(Vars, Constraints @ [Constraint | _]) :-
    ...
    ;
        ...
        DummyObjective = [Term],
        lpq.solve(Constraints, max, DummyObjective, Vars) =
            lp_res_inconsistent
    ).
...
lpq.set_vars_to_zero(Vars, Constraints) =
...
bounding_box(Varset, Constraints) = BoundingBox :-
    ...
    CallProject =
        (func(Var, Constrs0) = Constrs :-
            Result = lpq.project([Var], Varset, Constrs0),
            (
...
lpq.solve(Constraints, Direction, Objective, Varset) = Result :-
...
normalize_constraint(Var, Constraint0, Constraint) :-
    lpq.deconstruct_constraint(Constraint0, Terms0, Op0, Constant0),
...
entailed(Varset, Constraints, lte(Objective, Constant)) = Result :-
    SolverResult = lpq.solve(Constraints, max, Objective, Varset),
    (
...
entailed(Varset, Constraints, gte(Objective, Constant)) = Result :-
    SolverResult = lpq.solve(Constraints, min, Objective, Varset),
    (
